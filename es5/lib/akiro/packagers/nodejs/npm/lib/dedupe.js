...
Deduper.prototype.loadIdealTree = function (cb) {
  ...
  }], [this, this.finishTracker, 'loadAllDepsIntoIdealTree'], [this, function (next) {
...
Deduper.prototype.generateActionsToTake = function (cb) {
  ...
  chain([[this.newTracker(log, 'hoist', 1)], [hoistChildren, this.idealTree, this.differences], [this, this.finishTracker, 'hoist'], [this.newTracker(log, 'sort-actions', 1)], [this, function (next) {
    this.differences = sortActions(this.differences);
    next();
  }], [this, this.finishTracker, 'sort-actions'], [filterInvalidActions, this.where, this.differences], [checkPermissions, this.differences], [decomposeActions, this.differences, this.todo]], cb);
};
...
function hoistChildren_(tree, diff, seen, next) {
  ...
  asyncMap(tree.children, function (child, done) {
    ...
    if (better) {
      return chain([[remove, child, diff], [recalculateMetadata, tree, log]], done);
    }
    var hoistTo = earliestInstallable(tree, tree.parent, child['package']);
    if (hoistTo) {
      move(child, hoistTo, diff);
      chain([[recalculateMetadata, hoistTo, log], [hoistChildren_, child, diff, seen], [function (next) {
        moveRemainingChildren(child, diff);
        next();
      }]], done);
    } else {
