...
function shrinkwrapDeps(dev, problems, deps, tree, seen) {
  ...
  Object.keys(tree.missingDeps).forEach(function (name) {
    var invalid = tree.children.filter(function (dep) {
    ...
    if (invalid) {
      ...
      problems.push('missing: ' + name + '@' + tree['package'].dependencies[name] + (topname ? ', required by ' + topname : ''));
    }
  });
  tree.children.sort(function (aa, bb) {
    ...
    validatePeerDeps(child, function (tree, pkgname, version) {
      problems.push('peer invalid: ' + pkgname + '@' + version + ', required by ' + child['package']._id);
    });
