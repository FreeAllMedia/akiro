...
function getOnceFromRegistry(name, from, next, done) {
  function fixName(err, data, json, resp) {
    ...
    if (err && err.pkgid && err.pkgid !== name) {
      err.message = err.message.replace(new RegExp(': ' + err.pkgid.replace(/(\W)/g, '\\$1') + '$'), ': ' + name);
      err.pkgid = name;
    }
  ...
  mapToRegistry(name, npm.config, function (er, uri, auth) {
    ...
    if (!next) return log.verbose(from, key, 'already in flight; waiting');else log.verbose(from, key, 'not in flight; fetching');
...
function addNameTag(name, tag, data, cb) {
  ...
  function next(er, data, json, resp) {
    ...
    engineFilter(data);
    if (data['dist-tags'] && data['dist-tags'][tag] && data.versions[data['dist-tags'][tag]]) {
...
function engineFilter(data) {
  ...
  Object.keys(data.versions || {}).forEach(function (v) {
    ...
    if (!eng) return;
    if (!strict) return;
    if (eng.node && !semver.satisfies(nodev, eng.node, true) || eng.npm && !semver.satisfies(npmv, eng.npm, true)) {
    ...
    fs.stat(pkgtgz, function (er) {
      if (!er) {
        readJson(pkgjson, function (er, data) {
          ...
          if (er) return fetchit();else return cb(null, data);
        });
      } else return fetchit();
    });
...
function addNameRange(name, range, data, cb) {
  ...
  if (range === null) {
    return cb(new Error('Invalid version range: ' + range));
  }
  ...
  function next() {
    log.silly('addNameRange', 'number 2', { name: name, range: range, hasData: !!data });
    engineFilter(data);

    log.silly('addNameRange', 'versions', [data.name, Object.keys(data.versions || {})]);
