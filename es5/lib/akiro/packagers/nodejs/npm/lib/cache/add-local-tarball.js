...
function addTmpTarball(tgz, pkgData, shasum, cb) {
  ...
  if (pkgData && pkgData.name && pkgData.version) {
    log.verbose('addTmpTarball', 'already have metadata; skipping unpack for', packageId(pkgData));
    ...
    tar.unpack(tgz, target, null, null, cs.uid, cs.gid, function (unpackEr, data) {
      ...
      rimraf(target, function () {
        ...
        if (!data.version) {
          ...
          return cb(new Error('Invalid Package: expected ' + packageId(pkgData) + ' but found ' + packageId(data)));
        }
