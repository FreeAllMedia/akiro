...
function validateExistingRemote(from, cloneURL, treeish, cachedRemote, cb) {
  git.whichAndExec(['config', '--get', 'remote.origin.url'], { cwd: cachedRemote, env: gitEnv() }, function (er, stdout, stderr) {
    var originURL;
    if (stdout) {
...
function mirrorRemote(from, cloneURL, treeish, cachedRemote, silent, cb) {
  mkdir(cachedRemote, function (er) {
    ...
    git.whichAndExec(['clone', '--template=' + templates, '--mirror', cloneURL, cachedRemote], { cwd: cachedRemote, env: gitEnv() }, function (er, stdout, stderr) {
      if (er) {
        var combined = (stdout + '\n' + stderr).trim();
...
function resolveHead(from, cloneURL, treeish, cachedRemote, cb) {
  ...
  git.whichAndExec(args, { cwd: cachedRemote, env: gitEnv() }, function (er, stdout, stderr) {
    if (er) {
...
function checkoutTreeish(from, resolvedURL, resolvedTreeish, tmpdir, cb) {
  ...
  git.whichAndExec(args, { cwd: tmpdir, env: gitEnv() }, function (er, stdout, stderr) {
    stdout = (stdout + '\n' + stderr).trim();
    if (er) {
      log.error('git ' + args.join(' ') + ':', stderr);
      return cb(er);
