...
install.completion = function (opts, cb) {
  ...
  if (/\//.test(opts.partialWord)) {
    ...
    var annotatePackageDirMatch = function annotatePackageDirMatch(sibling, cb) {
...
Installer.prototype.run = function (cb) {
  ...
  if (!this.dryrun) {
    ...
    postInstallSteps.push([this.newTracker(log, 'rollbackFailedOptional', 1)], [this, this.rollbackFailedOptional, staging, this.todo], [this, this.finishTracker, 'rollbackFailedOptional'], [this, this.commit, staging, this.todo], [this.newTracker(log, 'runTopLevelLifecycles', 2)], [this, this.runTopLevelLifecycles], [this, this.finishTracker, 'runTopLevelLifecycles']);

    if (getSaveType(this.args)) {
      postInstallSteps.push([this, this.saveToDependencies]);
    }
  }
  postInstallSteps.push([this, this.printInstalled]);
...
Installer.prototype.loadCurrentTree = function (cb) {
  ...
  todo.push([this, this.normalizeTree, log.newGroup('normalizeTree')]);
  chain(todo, cb);
};
...
Installer.prototype.loadIdealTree = function (cb) {
  ...
  chain([[this.newTracker(this.progress.loadIdealTree, 'cloneCurrentTree')], [this, this.cloneCurrentTreeToIdealTree], [this, this.finishTracker, 'cloneCurrentTree'], [this.newTracker(this.progress.loadIdealTree, 'loadShrinkwrap')], [this, this.loadShrinkwrap], [this, this.finishTracker, 'loadShrinkwrap'], [this.newTracker(this.progress.loadIdealTree, 'loadAllDepsIntoIdealTree', 10)], [this, this.loadAllDepsIntoIdealTree], [this, this.finishTracker, 'loadAllDepsIntoIdealTree'], [this, function (next) {
    recalculateMetadata(this.idealTree, log, next);
  }], [this, this.debugTree, 'idealTree:prePrune', 'idealTree'], [this, function (next) {
    next(pruneTree(this.idealTree));
  }]], cb);
};
...
Installer.prototype.loadAllDepsIntoIdealTree = function (cb) {
  ...
  if (installNewModules) {
    ...
    if (this.prod) {
      steps.push([loadDeps, this.idealTree, cg.newGroup('loadDeps')]);
    }
    if (this.dev) {
      steps.push([loadDevDeps, this.idealTree, cg.newGroup('loadDevDeps')]);
    }
  }
...
Installer.prototype.generateActionsToTake = function (cb) {
  ...
  chain([[validateTree, this.idealTree, cg.newGroup('validateTree')], [diffTrees, this.currentTree, this.idealTree, this.differences, cg.newGroup('diffTrees')], [this, this.computeLinked], [filterInvalidActions, this.where, this.differences], [checkPermissions, this.differences], [decomposeActions, this.differences, this.todo]], cb);
};
...
Installer.prototype.computeLinked = function (cb) {
  ...
  asyncMap(this.differences, function (action, next) {
    ...
    var isReqByTop = pkg['package']._requiredBy.filter(function (name) {
      return name === '/';
    }).length;
    var isReqByUser = pkg['package']._requiredBy.filter(function (name) {
      return name === '#USER';
    }).length;
    ...
    isLinkable(pkg, function (install, link) {
      ...
      if (install || link) {
        pkg.parent.children = pkg.parent.children.filter(function (child) {
          return child !== pkg;
        });
      }
      next();
...
Installer.prototype.executeActions = function (cb) {
  ...
  if (this.npat) {
    steps.push([doParallelActions, 'test', staging, todo, trackLifecycle.newGroup('npat')]);
  }
...
Installer.prototype.runTopLevelLifecycles = function (cb) {
  ...
  steps.push([doOneAction, 'preinstall', this.idealTree.path, this.idealTree, trackLifecycle.newGroup('preinstall:.')], [doOneAction, 'build', this.idealTree.path, this.idealTree, trackLifecycle.newGroup('build:.')], [doOneAction, 'install', this.idealTree.path, this.idealTree, trackLifecycle.newGroup('install:.')], [doOneAction, 'postinstall', this.idealTree.path, this.idealTree, trackLifecycle.newGroup('postinstall:.')]);
  if (this.npat) {
    steps.push([doOneAction, 'test', this.idealTree.path, this.idealTree, trackLifecycle.newGroup('npat:.')]);
  }
  if (this.dev) {
    steps.push([doOneAction, 'prepublish', this.idealTree.path, this.idealTree, trackLifecycle.newGroup('prepublish')]);
  }
  chain(steps, cb);
};
