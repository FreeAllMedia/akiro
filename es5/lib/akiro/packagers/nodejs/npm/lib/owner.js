...
owner.completion = function (opts, cb) {
  ...
  if (argv.length <= 2) {
    var subs = ['add', 'rm'];
    if (opts.partialWord === 'l') subs.push('ls');else subs.push('ls', 'list');
    return cb(null, subs);
  }
...
function rm(user, pkg, cb) {
  ...
  mutate(pkg, user, function (u, owners) {
    ...
    if (!m.length) {
      return new Error('Cannot remove all owners of a package.  Add someone else first.');
    }
...
function mutate(pkg, user, mutation, cb) {
  ...
  function mutate_(er, u) {
    if (!er && user && (!u || u.error)) {
      er = new Error("Couldn't get user data for " + user + ': ' + JSON.stringify(u));
    }
