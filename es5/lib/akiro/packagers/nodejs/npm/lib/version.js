...
function version(args, silent, cb_) {
  ...
  fs.readFile(packagePath, function (er, data) {
    ...
    chain([[checkGit, localData], [lifecycle, lifecycleData, 'preversion', where], [updatePackage, newVersion, silent], [lifecycle, lifecycleData, 'version', where], [commit, localData, newVersion], [lifecycle, lifecycleData, 'postversion', where]], cb_);
  });
}
...
function checkGit(localData, cb) {
  fs.stat(path.join(npm.localPrefix, '.git'), function (er, s) {
    ...
    // check for git
    git.whichAndExec(['status', '--porcelain'], { env: process.env }, function (er, stdout) {
...
function _commit(version, hasShrinkwrap, cb) {
  ...
  var flag = sign ? '-sm' : '-am';
  chain([git.chainableExec(['add', packagePath], options), hasShrinkwrap && git.chainableExec(['add', 'npm-shrinkwrap.json'], options), git.chainableExec(['commit', '-m', message], options), git.chainableExec(['tag', npm.config.get('tag-version-prefix') + version, flag, message], options)], cb);
}
...
function write(data, file, cb) {
  ...
  writeFileAtomic(path.join(npm.localPrefix, file), new Buffer(JSON.stringify(data, null, 2) + '\n'), cb);
}