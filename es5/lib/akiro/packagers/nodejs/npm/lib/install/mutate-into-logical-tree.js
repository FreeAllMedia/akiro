...
var mutateIntoLogicalTree = module.exports = function (tree) {
  ...
  function getNode(flatname) {
    return flatname.substr(0, 5) === '#DEV:' ? flat[flatname.substr(5)] : flat[flatname];
  }
  ...
  Object.keys(flat).sort().forEach(function (flatname) {
    ...
    var requiredByNames = requiredBy.filter(function (parentFlatname) {
      ...
      return parentNode['package'].dependencies[moduleName(node)] || parentNode['package'].devDependencies && parentNode['package'].devDependencies[moduleName(node)];
    });
    ...
    if (node['package']._requiredBy.some(function (nodename) {
      return nodename[0] === '#';
    })) {
