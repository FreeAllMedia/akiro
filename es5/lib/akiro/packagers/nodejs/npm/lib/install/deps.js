...
function recalculateMetadata(tree, log, seen, next) {
  ...
  chain([[asyncMap, tomark, markDeps], [asyncMap, tree.children, function (child, done) {
    recalculateMetadata(child, log, seen, done);
  }]], function () {
    tree.userRequired = tree['package']._requiredBy.some(function (req) {
      return req === '#USER';
    });
    tree.existing = tree['package']._requiredBy.some(function (req) {
      return req === '#EXISTING';
    });
  ...
  asyncMap(args, function (spec, done) {
    if (tree && spec.lastIndexOf('@') <= 0) {
      ...
      if (sw) {
        ...
        spec = sw.resolved ? spec + '@' + sw.resolved : sw.from && url.parse(sw.from).protocol ? spec + '@' + sw.from : spec + '@' + sw.version;
      } else {
  ...
  asyncMap(args, function (pkg, done) {
    ...
    resolveWithNewModule(pkg, tree, log.newGroup('loadRequestedDeps'), iferr(depLoaded, function (child, tracker) {
      ...
      if (saveToDependencies) {
        tree['package'][saveToDependencies][childName] = child['package']._requested.rawSpec || child['package']._requested.spec;
      }
      if (saveToDependencies && saveToDependencies !== 'devDependencies') {
        tree['package'].dependencies[childName] = child['package']._requested.rawSpec || child['package']._requested.spec;
      }
...
function loadDeps(tree, log, next) {
  ...
  asyncMap(Object.keys(tree['package'].dependencies), function (dep, done) {
    ...
    if (tree['package'].optionalDependencies && tree['package'].optionalDependencies[dep] && !npm.config.get('optional')) {
      return done();
    }
...
var findRequirement = exports.findRequirement = function (tree, name, requested, requestor) {
  ...
  var nameMatch = function nameMatch(child) {
    return moduleName(child) === name && child.parent && !child.removed;
  };
  var versionMatch = function versionMatch(child) {
