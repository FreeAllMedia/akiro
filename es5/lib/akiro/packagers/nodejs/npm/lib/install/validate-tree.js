...
module.exports = function (idealTree, log, next) {
  ...
  }], [thenValidateAllPeerDeps, idealTree], [thenCheckTop, idealTree]], andFinishTracker(log, next));
};
...
function thenValidateAllPeerDeps(idealTree, next) {
  ...
  validateAllPeerDeps(idealTree, function (tree, pkgname, version) {
    var warn = new Error(packageId(tree) + ' requires a peer of ' + pkgname + '@' + version + ' but none was installed.');
