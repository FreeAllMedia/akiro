...
function saveShrinkwrap(tree, next) {
  ...
  fs.stat(saveTarget, function (er, stat) {
    ...
    var hasDevOnlyDeps = tree.requires.filter(function (dep) {
      var devReqs = dep['package']._requiredBy.filter(function (name) {
        return name.substr(0, 4) === '#DEV';
      });
...
var getSaveType = exports.getSaveType = function (args) {
  ...
  var noSaveFlags = !npm.config.get('save') && !npm.config.get('save-dev') && !npm.config.get('save-optional');
  if (nothingToSave || globalInstall || noSaveFlags) return null;

  if (npm.config.get('save-optional')) return 'optionalDependencies';else if (npm.config.get('save-dev')) return 'devDependencies';else return 'dependencies';
};
