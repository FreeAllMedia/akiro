...
var inflateShrinkwrap = module.exports = function (tree, swdeps, finishInflating) {
  ...
  asyncMap(Object.keys(swdeps), function (name, next) {
    var sw = swdeps[name];
    var spec = sw.resolved ? name + '@' + sw.resolved : sw.from && url.parse(sw.from).protocol ? name + '@' + sw.from : name + '@' + sw.version;
    var child = onDisk[name];
    if (child && (child.fromShrinkwrap || sw.resolved && child['package']._resolved === sw.resolved || sw.from && url.parse(sw.from).protocol && child['package']._from === sw.from || child['package'].version === sw.version)) {
