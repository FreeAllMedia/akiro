...
function fetchAndRead(nv, args, silent, cb) {
  ...
  mapToRegistry(name, npm.config, function (er, uri, auth) {
    ...
    npm.registry.get(uri, { auth: auth }, function (er, data) {
      ...
      if (error || silent) cb(error, retval);else printData(results, data._id, cb.bind(null, error, retval));
    });
  });
}
...
function showFields(data, version, fields) {
  var o = {};[data, version].forEach(function (s) {
  ...
  _function: while (_again) {
    ...
    if (Array.isArray(data)) {
      ...
      data.forEach(function (data, i) {
        var tl = title.length;
        var newt = title.substr(0, tl - fields.join('.').length - 1) + '[' + i + ']' + [''].concat(fields).join('.');
    ...
    if (tail.length) {
      if (typeof data === 'object') {
        ...
        _x4 = title;
        _again = true;
        field = tail = o = index = results = undefined;
        continue _function;
      } else {
  ...
  versions.forEach(function (v) {
    ...
    fields.forEach(function (f) {
      ...
      if (includeVersions || includeFields || typeof d !== 'string') {
        d = cleanup(data[v][f]);
        d = npm.config.get('json') ? JSON.stringify(d, null, 2) : util.inspect(d, false, 5, npm.color);
...
function cleanup(data) {
  ...
  if (typeof data.versions === 'object' && data.versions && !Array.isArray(data.versions)) {
    data.versions = Object.keys(data.versions || {});
  }

  var keys = Object.keys(data);
  keys.forEach(function (d) {
    if (d.charAt(0) === '_') delete data[d];else if (typeof data[d] === 'object') data[d] = cleanup(data[d]);
  });
