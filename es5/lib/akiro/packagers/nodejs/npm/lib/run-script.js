...
runScript.completion = function (opts, cb) {
  ...
  if (argv.length === 3) {
    ...
    return readJson(json, function (er, d) {
      ...
      var pref = npm.config.get('global') ? npm.config.get('prefix') : npm.localPrefix;
...
function list(cb) {
  var json = path.join(npm.localPrefix, 'package.json');
  var cmdList = ['publish', 'install', 'uninstall', 'test', 'stop', 'start', 'restart', 'version'].reduce(function (l, p) {
  ...
  return readJson(json, function (er, d) {
    ...
    allScripts.forEach(function (script) {
      if (cmdList.indexOf(script) !== -1) scripts.push(script);else runScripts.push(script);
    });
...
function run(pkg, wd, cmd, args, cb) {
  ...
  if (cmd === 'restart' && !pkg.scripts.restart) {
    cmds = ['prestop', 'stop', 'poststop', 'restart', 'prestart', 'start', 'poststart'];
  } else {
    if (!pkg.scripts[cmd]) {
      if (cmd === 'test') {
