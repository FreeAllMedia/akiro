...
function lifecycle(pkg, stage, wd, unsafe, failOk, cb) {
  ...
  validWd(wd || path.resolve(npm.dir, pkg.name), function (er, wd) {
    ...
    if ((wd.indexOf(npm.dir) !== 0 || wd.indexOf(pkg.name) !== wd.length - pkg.name.length) && !unsafe && pkg.scripts[stage]) {
      log.warn('lifecycle', logid(pkg, stage), 'cannot run in wd', '%s %s (wd=%s)', pkg._id, pkg.scripts[stage], wd);
      return cb();
    }
...
function lifecycle_(pkg, stage, wd, env, unsafe, failOk, cb) {
  ...
  chain([packageLifecycle && [runPackageLifecycle, pkg, env, wd, unsafe], [runHookLifecycle, pkg, env, wd, unsafe]], done);
}
  ...
  for (i in data) if (i.charAt(0) !== '_') {
    ...
    if (data[i] && typeof data[i] === 'object') {
      try {
        ...
        var d = data[i];
        makeEnv({ name: d.name, version: d.version, path: d.path }, envKey + '_', env);
      }
    } else {
