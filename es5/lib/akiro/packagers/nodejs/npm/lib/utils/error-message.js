...
function errorMessage(er) {
  ...
  switch (er.code) {
    ...
    case 'EBADPLATFORM':
      short.push(['notsup', ['Not compatible with your operating system or architecture: ' + er.pkgid].join('\n')]);
    ...
    case 'EAI_FAIL':
      ...
      detail.push(['network', ['This is most likely not a problem with npm itself', 'and is related to network connectivity.', 'In most cases you are behind a proxy or have bad network settings.', '\nIf you are behind a proxy, please make sure that the', "'proxy' config is set properly.  See: 'npm help config'"].join('\n')]);
      break;

    case 'ENOPACKAGEJSON':
      short.push(['package.json', er.message]);
      detail.push(['package.json', ['This is most likely not a problem with npm itself.', "npm can't find a package.json file in your current directory."].join('\n')]);
      break;

    case 'ETARGET':
      short.push(['notarget', er.message]);
      msg = ['This is most likely not a problem with npm itself.', 'In most cases you or one of your dependencies are requesting', "a package version that doesn't exist."];
      if (er.parent) {
    ...
    case 'ENOSPC':
      ...
      detail.push(['nospc', ['This is most likely not a problem with npm itself', 'and is related to insufficient space on your system.'].join('\n')]);
      break;

    case 'EROFS':
      short.push(['rofs', er.message]);
      detail.push(['rofs', ['This is most likely not a problem with npm itself', 'and is related to the file system being read-only.', '\nOften virtualized file systems, or other file systems', "that don't support symlinks, give this error."].join('\n')]);
      break;

    case 'ENOENT':
      short.push(['enoent', er.message]);
      detail.push(['enoent', [er.message, 'This is most likely not a problem with npm itself', 'and is related to npm not being able to find a file.', er.file ? "\nCheck if the file '" + er.file + "' is present." : ''].join('\n')]);
      break;
    ...
    case 'ETOOMANYARGS':
      ...
      detail.push(['typeerror', ['This is an error with npm itself. Please report this error at:', '    <http://github.com/npm/npm/issues>'].join('\n')]);
      break;

    case 'EISDIR':
      short.push(['eisdir', er.message]);
      detail.push(['eisdir', ['This is most likely not a problem with npm itself', 'and is related to npm not being able to find a package.json in', 'a package you are trying to install.'].join('\n')]);
      break;

    default:
      short.push(['', er.message || er]);
      detail.push(['', ['', 'If you need help, you may report this error at:', '    <https://github.com/npm/npm/issues>'].join('\n')]);
      break;
  }
