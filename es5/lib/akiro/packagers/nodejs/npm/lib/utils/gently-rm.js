...
function gentlyRm(target, gently, base, cb) {
  ...
  log.silly('gentlyRm', target, 'is being', gently ? 'gently removed' : 'purged', base ? 'from base ' + base : '');
  ...
  // installed globally
  var prefixes = [npm.prefix, npm.globalPrefix, npm.dir, npm.root, npm.globalDir, npm.bin, npm.globalBin];
  ...
  // safe operation
  chain([[isEverInside, parent, prefixes], [readLinkOrShim, targetPath], [isEverInside, targetPath, prefixes], [isEverInside, targetPath, [parent]]], function (er, results) {
    if (er) {
    ...
    function thenRemove(toRemove, removeBase) {
      ...
      log.verbose('gentlyRm', options.purge ? 'Purging' : 'Vacuuming', toRemove, 'up to', options.base);
...
function isSafeToRm(parent, target, cb) {
  ...
  if (!parent.managed) {
    log.verbose('gentlyRm', parent.path, 'is not contained in any diretory npm is known to control or ' + 'any place they link to');
    return cb(clobberFail(target.path, 'containing path ' + parent.path + " isn't under npm's control"));
  }
  ...
  if (target.managed && target.symlink) {
    log.warn('gentlyRm', 'not removing', target.path, "as it wasn't installed by", parent.path);
    return cb();
  }

  if (target.symlink) {
    return cb(clobberFail(target.path, target.symlink + ' symlink target is not controlled by npm ' + parent.path));
  } else {
