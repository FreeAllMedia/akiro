...
process.on('exit', function (code) {
  ...
  if (itWorked) log.info('ok');else {
    ...
    if (wroteLogFile) {
      ...
      log.error('', ['Please include the following file with any support request:', '    ' + path.resolve('npm-debug.log')].join('\n'));
      wroteLogFile = false;
    }
    if (code) {
      log.error('code', code);
  ...
  if (rollbacks.length) {
    chain(rollbacks.map(function (f) {
      ...
      if (er) {
        ...
        if (!code) errorHandler(er);else if (noLog) rm('npm-debug.log', reallyExit.bind(null, er));else writeLogFile(reallyExit.bind(this, er));
      } else {
        if (!noLog && code) writeLogFile(reallyExit);else rm('npm-debug.log', reallyExit);
      }
    });
    rollbacks.length = 0;
  } else if (code && !noLog) writeLogFile(reallyExit);else rm('npm-debug.log', reallyExit);
...
function errorHandler(er) {
  ...
  ;['type', 'fstream_path', 'fstream_unc_path', 'fstream_type', 'fstream_class', 'fstream_finish_call', 'fstream_linkpath', 'stack', 'fstream_stack', 'statusCode', 'pkgid'].forEach(function (k) {
    var v = er[k];
    if (!v) return;
  ...
  log.error('npm ', 'v' + npm.version);['file', 'path', 'code', 'errno', 'syscall'].forEach(function (k) {
    var v = er[k];
    if (v) log.error(k, v);
  });
