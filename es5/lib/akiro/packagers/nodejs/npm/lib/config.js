...
function edit(cb) {
  ...
  npm.config.save(which, function (er) {
    ...
    fs.readFile(f, 'utf8', function (er, data) {
      if (er) data = '';
      data = [';;;;', '; npm ' + (npm.config.get('global') ? 'globalconfig' : 'userconfig') + ' file', '; this is a simple ini-formatted file', '; lines that start with semi-colons are comments.', '; read `npm help config` for help on the various options', ';;;;', '', data].concat([';;;;', '; all options with default values', ';;;;']).concat(Object.keys(npmconf.defaults).reduce(function (arr, key) {
        var obj = {};
        obj[key] = npmconf.defaults[key];
        if (key === 'logstream') return arr;
        return arr.concat(ini.stringify(obj).replace(/\n$/m, '').replace(/^/g, '; ').replace(/\n/g, '\n; ').split('\n'));
      }, [])).concat(['']).join(os.EOL);
...
function publicVar(k) {
  return !(k.charAt(0) === '_' || k.indexOf(':_') !== -1 || types[k] !== types[k]);
}
...
function list(cb) {
  ...
  if (uconfKeys.length) {
    ...
    uconfKeys.forEach(function (k) {
      var val = k.charAt(0) === '_' ? '---sekretz---' : JSON.stringify(uconf[k]);
  ...
  if (gconfKeys.length) {
    ...
    gconfKeys.forEach(function (k) {
      var val = k.charAt(0) === '_' ? '---sekretz---' : JSON.stringify(gconf[k]);
  ...
  if (builtin && builtin.data) {
    ...
    if (bconfKeys.length) {
      ...
      bconfKeys.forEach(function (k) {
        var val = k.charAt(0) === '_' ? '---sekretz---' : JSON.stringify(bconf[k]);
