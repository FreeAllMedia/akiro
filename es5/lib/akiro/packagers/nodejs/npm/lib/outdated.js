  ...
  readPackageTree(dir, andRecalculateMetadata(function (er, tree) {
    ...
    outdated_(args, '', tree, {}, 0, function (er, list) {
      ...
      if (npm.config.get('json')) {
        ...
        var outHead = ['Package', 'Current', 'Wanted', 'Latest', 'Location'];
      ...
      cb(null, list.map(function (item) {
...
function makePretty(p) {
  ...
  var columns = [depname, has || 'MISSING', want, latest, deppath];
...
function makeParseable(list) {
  return list.map(function (p) {
    ...
    var out = [dir, depname + '@' + want, has ? depname + '@' + has : 'MISSING', depname + '@' + latest];
...
function outdated_(args, path, tree, parentHas, depth, cb) {
  ...
  var deps = tree.children.filter(function (child) {
    return !isExtraneous(child);
  }) || [];
  ...
  if ((dev || !prod) && (npm.config.get('save-dev') || !npm.config.get('save') && !npm.config.get('save-optional'))) {
  ...
  var doUpdate = dev || !prod && !Object.keys(parentHas).length && !npm.config.get('global');
  if (doUpdate) {
  ...
  asyncMap(deps, function (dep, cb) {
    ...
    var required = tree['package'].dependencies[name] || tree['package'].optionalDependencies[name] || tree['package'].devDependencies[name] || dep['package']._requested && dep['package']._requested.spec || '*';
...
function shouldUpdate(args, tree, dep, has, req, depth, pkgpath, cb, type) {
  ...
  function skip(er) {
    ...
    if (er) return cb(er);
    outdated_(args, pkgpath, tree, has, depth + 1, cb);
  }
  ...
  function updateDeps(er, d) {
    ...
    function onCacheAdd(er, d) {
      ...
      if (!curr || dFromUrl && cFromUrl && d._from !== curr.from || d.version !== curr.version || d.version !== l.version) {
