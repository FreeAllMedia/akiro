...
function stripData(data) {
  return {
    ...
    time: data.time && data.time.modified && new Date(data.time.modified).toISOString() // remove time
    .split('T').join(' ').replace(/:[0-9]{2}\.[0-9]{3}Z$/, '').slice(0, -5) || 'prehistoric'
  };
}

function getWords(data) {
  data.words = [data.name].concat(data.description).concat(data.maintainers).concat(data.url && '<' + data.url + '>').concat(data.keywords).map(function (f) {
...
function prettify(data, args) {
  ...
  var columns = npm.config.get('description') ? ['name', 'description', 'author', 'date', 'version', 'keywords'] : ['name', 'author', 'date', 'version', 'keywords'];
...
function addColorMarker(str, arg, i) {
  ...
  if (arg.charAt(0) === '/') {
    return str.replace(new RegExp(arg.substr(1, arg.length - 2), 'gi'), function (bit) {
